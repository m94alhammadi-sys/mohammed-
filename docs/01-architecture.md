...
|---|---|---|
| مزوّد واحد لموصل يسقط | انتقال تلقائي إلى المسار الاحتياطي | لا أثر — الموصل ينجح |
| عطل عابر (مهلة، 429، 5xx) | إعادة محاولة بتراجع أسّي (٣ محاولات) | لا أثر غالباً |
| رفض دائم (401/403/404) | فشل فوري بلا تكرار، مصنّف «محجوب» | يظهر في `data_gaps` |
| مفتاح غير مضبوط | فشل مصنّف `MISSING_KEY` | نقص تغطية معلن — **لا يرفع `degraded`** |
| كل مسارات موصل ميتة | `ConnectorResult.ok=False` بسبب مقروء | يظهر في `data_gaps` |
